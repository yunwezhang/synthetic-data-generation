...
# 500 run stat simulation
```{r eval=FALSE}
vitd_sim=function(seed,data){
vitd1=data
modgo_dt=vitd1[,!colnames(vitd1)%in%c("bwt")]
modgo_dt[,c(3,4)]=modgo_dt[,c(4,3)]
colnames(modgo_dt)[3:4]=c("smoke","race")
# a similar n
test <- modgo(data = modgo_dt,bin_variables = c("smoke"),categ_variables = c("race"),nrep = 1,ties_method=  "max", variables= colnames(modgo_dt),
                  n_samples= 189,sigma= c(),
                  noise_mu= FALSE, pertr_vec= c(),
...
start_time <- Sys.time()
result1=pbmcapply::pbmclapply(1:500, vitd_sim,data=read.csv("bwt_3_scaled_all.csv"),mc.cores = 15)
end_time <- Sys.time()
runtime <- end_time - start_time
runtime
saveRDS(result1,"bwt_stat_500.rds")
...
start_time <- Sys.time()
result1=pbmcapply::pbmclapply(1:500, heart_sim,data=read_csv("breast3.csv"),mc.cores = 15)
end_time <- Sys.time()
runtime <- end_time - start_time
runtime
saveRDS(result1,"breast_stat_500.rds")

heart_sim2=function(seed,data){
  heartdt=data
modgo_dt=heartdt[,!colnames(heartdt)=="outcome"]
colnames(modgo_dt)
modgo_dt=as.data.frame(modgo_dt)
...
start_time <- Sys.time()
result1=pbmcapply::pbmclapply(1:500, heart_sim2,data=read_csv("breast3.csv"),mc.cores = 15)
end_time <- Sys.time()
runtime <- end_time - start_time
runtime
saveRDS(result1,"breast_stat2_500.rds")
...
test <- modgo(data = modgo_dt,bin_variables = bin_variables,categ_variables = categ_variables,nrep = 1,ties_method=  "max", variables= colnames(modgo_dt),
                  n_samples= dim(modgo_dt)[1],sigma= c(),
                  noise_mu= FALSE, pertr_vec= c(),
                  change_cov= c(),change_amount= 0,seed= seed,
                  thresh_var= c(), thresh_force = FALSE,
                  var_prop= c(),var_infl=c(),infl_cov_stable=FALSE)
modgo_new1=test[[1]][[1]]
veteran1_stat=sim_fun_cox(dat=veteran3,seed=seed,n=dim(modgo_dt)[1],X=modgo_new1)
  return(veteran1_stat)
}
start_time <- Sys.time()
result1=pbmcapply::pbmclapply(1:500, veteran_sim,data=read_csv("lung3.csv"),mc.cores = 1)
end_time <- Sys.time()
runtime <- end_time - start_time
runtime
saveRDS(result1,"lung_stat_500.rds")
...
## real data bwt
```{r}
vitd_real=read.csv("bwt_3_scaled_all.csv")#6 categorical vars#7 continuous var
continuous_var=c("age", "lwt" )
cat_var=c("race", "smoke")
library(moments)
subset_data <- as.data.frame(vitd_real[, continuous_var])
calculate_moments <- function(x) {
  ...
  return(c(mean = mean_x, variance = variance_x, skewness = skewness_x, kurtosis = kurtosis_x))
}
#var(as.vector(subset_data[,1]))
vitd_moments_df <- as.data.frame(t(sapply(subset_data, calculate_moments)))
vitd_moments_df

calculate_proportions <- function(column, var_name) {
  prop_table <- prop.table(table(column))
  prop_df <- as.data.frame(prop_table)
  prop_df$Variable <- var_name
  colnames(prop_df) <- c("Level", "Proportion", "Variable")
  return(prop_df)
}
vitd_proportions <- data.frame(Level = character(), Proportion = numeric(), Variable = character(), stringsAsFactors = FALSE)
for (var in cat_var) {
  prop_df <- calculate_proportions(vitd_real[[var]], var)
  vitd_proportions <- rbind(vitd_proportions, prop_df)
}
vitd_proportions
...
## stat results bwt
```{r}
vitd_list_of_dataframes=readRDS("bwt_stat_500.rds")
calculate_proportions_for_dataset <- function(df, cat_vars) {
  combined_proportions <- data.frame(Level = character(), Proportion = numeric(), Variable = character(), stringsAsFactors = FALSE)
  for (var in cat_var) {
    prop_df <- calculate_proportions(df[[var]], var)
    combined_proportions <- rbind(combined_proportions, prop_df)
  }
  return(combined_proportions)
}
# Initialize an empty data frame to store the sum of proportions
summed_proportions <- data.frame(Level = character(), Proportion = numeric(), Variable = character(), stringsAsFactors = FALSE)
# Calculate and aggregate proportions for each dataset
for (i in 1:length(vitd_list_of_dataframes)) {
  dataset_proportions <- calculate_proportions_for_dataset(vitd_list_of_dataframes[[i]], cat_var)
  if (nrow(summed_proportions) == 0) {
    summed_proportions <- dataset_proportions
  } else {
    summed_proportions <- merge(summed_proportions, dataset_proportions, by = c("Level", "Variable"), all = TRUE)
    summed_proportions$Proportion <- rowSums(summed_proportions[, grepl("Proportion", colnames(summed_proportions))], na.rm = TRUE)
    summed_proportions <- summed_proportions[, c("Level", "Proportion", "Variable")]
  }
}
summed_proportions$Average_Proportion <- summed_proportions$Proportion / length(vitd_list_of_dataframes)
vitd_summed_proportions=summed_proportions

# Function to perform chi-square test for each variable
perform_chi_square_test <- function(real_proportions, summed_proportions, num_datasets) {
  # Initialize an empty list to store chi-square test results
  chi_square_results <- list()
  
  # Iterate over each unique variable
  unique_vars <- unique(summed_proportions$Variable)
  for (var in unique_vars) {
    # Extract proportions for the current variable
    real_prop <- real_proportions[real_proportions$Variable == var, ]
    summed_prop <- summed_proportions[summed_proportions$Variable == var, ]
    
    # Calculate observed frequencies from real proportions
    observed_frequencies <- real_prop$Proportion * num_datasets
    
    # Calculate expected frequencies from average proportions
    expected_frequencies <- summed_prop$Average_Proportion * num_datasets
    
    # Perform chi-square test
    chi_square_test <- chisq.test(x = observed_frequencies, p = expected_frequencies / sum(expected_frequencies))
    
    # Store the result
    chi_square_results[[var]] <- chi_square_test
  }
...
num_datasets <- 500  # Number of datasets used to calculate summed proportions
chi_square_results <- perform_chi_square_test(vitd_proportions, vitd_summed_proportions, num_datasets)

# Print the results
vitd_chi_square_results=chi_square_results

vitd_chi_square_results
```
...
moments_list <- lapply(vitd_list_of_dataframes, function(df) {
  sapply(df[continuous_var], calculate_moments)
})
all_moments_df <- do.call(rbind, lapply(moments_list, function(moments) {
  as.data.frame(t(moments))
}))
all_moments_df <- cbind(Variable = rep(continuous_var, each = 500), all_moments_df)
all_moments_long <- pivot_longer(all_moments_df, -Variable, names_to = "Moment", values_to = "Value")
summary_stats <- all_moments_long %>%
  group_by(Variable, Moment) %>%
...
```{r}
vitd_moments_df=as.data.frame(t(vitd_moments_df))
# Initialize lists to store vectors for each moment of each variable
mean_vectors <- vector("list", 2)
variance_vectors <- vector("list", 2)
skewness_vectors <- vector("list", 2)
kurtosis_vectors <- vector("list", 2)

# Loop through each data frame in the list
for (i in seq_along(moments_list)) {
  df <- moments_list[[i]]
  
  # Check if df has the correct dimensions (4 rows x 7 columns)
  if (nrow(df) == 4 && ncol(df) == 2) {
    # Loop through each variable
    for (j in 1:2) {
      var_name <- colnames(df)[j]
      
      # Append values for each moment to the corresponding vector
      mean_vectors[[j]] <- c(mean_vectors[[j]], df[1, j])
      variance_vectors[[j]] <- c(variance_vectors[[j]], df[2, j])
      skewness_vectors[[j]] <- c(skewness_vectors[[j]], df[3, j])
      kurtosis_vectors[[j]] <- c(kurtosis_vectors[[j]], df[4, j])
    }
  } else {
    warning(paste("Data frame", i, "does not have the expected 4x7 dimensions."))
  }
}

t_test_fun=function(mu,vector){
  test_result <- t.test(vector, mu = mu)
  return(test_result)
}

t_test_results1 <- list()
# Loop through each variable (assuming the order in mean_vectors corresponds to columns in vitd_moments_df)
for (i in 1:2) {
  # Get the population mean for the current variable from vitd_moments_df
  population_mean <- vitd_moments_df[1, i]  # Assuming the population means are in the first row, each column i
  
  # Get the corresponding mean vector for the current variable
  mean_vector <- mean_vectors[[i]]
  
  # Perform the t-test
  test_result <- t_test_fun(mu = population_mean, vector = mean_vector)
  
  # Store the result
  t_test_results1[[paste0("Variable", i)]] <- test_result
}

t_test_results2 <- list()

# Loop through each variable (assuming the order in var_vectors corresponds to columns in vitd_moments_df)
for (i in 1:2) {
  # Get the population mean for the current variable from the second row of vitd_moments_df
  population_mean <- vitd_moments_df[2, i]  # Get the value from the second row, i-th column
  
  # Get the corresponding variance vector for the current variable
  var_vector <- variance_vectors[[i]]
  
  # Perform the t-test
  test_result <- t_test_fun(mu = population_mean, vector = var_vector)
  
  # Store the result
  t_test_results2[[paste0("Variable", i)]] <- test_result
}

t_test_results3 <- list()
...
for (i in 1:2) {
  ...
  # Get the corresponding variance vector for the current variable
  var_vector <- skewness_vectors[[i]]
  
  # Perform the t-test
  test_result <- t_test_fun(mu = population_mean, vector = var_vector)
  
  # Store the result
  t_test_results3[[paste0("Variable", i)]] <- test_result
}

t_test_results4 <- list()
...
for (i in 1:2) {
  ...
  # Get the corresponding variance vector for the current variable
  var_vector <- kurtosis_vectors[[i]]
  
  # Perform the t-test
  test_result <- t_test_fun(mu = population_mean, vector = var_vector)
  
  # Store the result
  t_test_results4[[paste0("Variable", i)]] <- test_result
}

# Combine results into a list
all_results <- list(
  "Mean" = t_test_results1,
  "Variance" = t_test_results2,
  "Skewness" = t_test_results3,
  "Kurtosis" = t_test_results4
)
...
## real data breast
```{r}
vitd_real=read.csv("breast3.csv")#6 categorical vars#7 continuous var
continuous_var=colnames(vitd_real)[2:31]
...
## stat results breast
```{r}
vitd_list_of_dataframes=readRDS("breast_stat_500.rds")
...
## real data lung
```{r}
vitd_real=read.csv("lung3.csv")#6 categorical vars#7 continuous var
continuous_var=c("age", "ph.karno","pat.karno","meal.cal","wt.loss")
cat_var=c("sex", "ph.ecog")
library(moments)
subset_data <- as.data.frame(vitd_real[, continuous_var])
calculate_moments <- function(x) {
  mean_x <- mean(x, na.rm = TRUE)
...
## stat results lung
```{r}
vitd_list_of_dataframes=readRDS("lung_stat_500.rds")
calculate_proportions_for_dataset <- function(df, cat_vars) {
  combined_proportions <- data.frame(Level = character(), Proportion = numeric(), Variable = character(), stringsAsFactors = FALSE)
  for (var in cat_var) {
    prop_df <- calculate_proportions(df[[var]], var)
...
# # Print the results
# vitd_chi_square_results=chi_square_results
#stat missing one category here
...
vitd_moments_df=as.data.frame(t(vitd_moments_df))
# Initialize lists to store vectors for each moment of each variable
mean_vectors <- vector("list", 5)
variance_vectors <- vector("list", 5)
skewness_vectors <- vector("list", 5)
kurtosis_vectors <- vector("list", 5)
