...
```{r}
heartdt=heart1
lm_model <- glm(target ~ age, data = heartdt, family=binomial())
summary(lm_model)
...
# Simulate new data based on the regression model
set.seed(20240421) # Set seed for reproducibility
fertility_new1 <- data.frame(age = rnorm(1000, mean = mean(heartdt$age), sd = sd(heartdt$age))) # Simulate new values for the predictor variable
probs<- predict(lm_model, newdata = fertility_new1, type = "response")
target <- rbinom(n = 1000, size = 1, prob = probs)
fertility_new1=cbind.data.frame(fertility_new1,target)
#write_csv(fertility_new1,file="heart_1_stat.csv")
...
```{r}
heartdt=heart2
formula_str <- paste(setdiff(colnames(heartdt), "target"), collapse = " + ")
formula_obj <- as.formula(paste("target ~", formula_str))
lm_model <- glm(formula_obj, data = heartdt, family=binomial())
summary(lm_model)

beta <- coef(lm_model)[-1]  # Exclude intercept
X=as.matrix(heartdt[,!colnames(heartdt)=="diagnosis"])
cor_matrix <- cor(X,use="na.or.complete")
mu=colMeans(X,na.rm = TRUE)
# Simulate new data based on the regression model
set.seed(20240421) # Set seed for reproducibility
new_X <- mvrnorm(1000, mu = mu, Sigma = cor_matrix)

probs<- predict(lm_model, newdata = as.data.frame(new_X), type = "response")
target <- rbinom(n = 1000, size = 1, prob = probs)
fertility_new1=cbind.data.frame(as.data.frame(new_X),target)
#write_csv(fertility_new1,file="heart_2_stat.csv")
```
...
test <- modgo(data = modgo_dt,bin_variables = c(),categ_variables = c(),nrep = 1,ties_method=  "max", variables= colnames(modgo_dt),
                  n_samples= 1000,sigma= c(),
                  noise_mu= FALSE, pertr_vec= c(),
                  change_cov= c(),change_amount= 0,seed= 1,
                  thresh_var= c(), thresh_force = FALSE,
...
#write_csv(vitd1_new1,file="heart_2_stat_compared.csv")
```
...
### continuous var
```{r }
vitd1=read.csv("heart2.csv")
vitd1_new1=read.csv("heart_2_stat.csv")
vitd1_new1$target=vitd1_new1$target.1
vitd1_ctgan1=read.csv("heart2_sim1.csv")
...
ggplot(df, aes(x = value, fill = group)) +
  ...
  scale_fill_manual(values = c("red", "blue", "green"))+theme_bw()
ggplot(df, aes(x = value, color = group, linetype = group)) +
  geom_density(size = 1) +
  labs(title = "Distribution of age", x = "Value", y = "Density") +
  scale_color_manual(values = c("red", "blue", "green")) +
  theme_bw() +
...
#ggsave("distribution.pdf",ff,height = 10,width = 5)
# distribution of response 
...
train_and_test_glm <- function(train_data, test_data) {
  # Train a linear regression model on the training data
formula_str <- paste(colnames(vitd1)[1:7], collapse = " + ")
formula_obj <- as.formula(paste("target ~", formula_str))
...
variables=c("age","cp","trestbps" ,"chol","thalach","oldpeak","ca","target")
cor1 <- cor(as.matrix(vitd1[,variables]),use = "pairwise.complete.obs")
  cor2 <- cor(as.matrix(vitd1_new1[,variables]),use = "pairwise.complete.obs")
  cor3 <- cor(as.matrix(vitd1_ctgan1[,variables]),use = "pairwise.complete.obs")
 par(mfrow = c(1, 3))
...
```{r}
vitd1=read.csv("heart2.csv")
vitd1_new1=read.csv("heart_2_stat_compared.csv")
vitd1_ctgan1=read.csv("heart2_sim1.csv")
...
g3=ggplot(df, aes(x = value, color = group, linetype = group)) +
  ...
  xlim(-4, 4)
ff=grid.arrange(g1,g2,g3,ncol=1)
#ggsave("distribution_compared.pdf",ff,height = 10,width = 5)
```
